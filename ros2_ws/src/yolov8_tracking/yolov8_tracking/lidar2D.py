...
class LidarCam():
    ...
    def projection(self, img, points, rslidar, cam_transform, P):
        """
        Performs lidar projection on image.
        Args:
        img (np.array) : image on which the lidar project has to be performed.
        points (np.array): Lidar point cloud 
        rslidar (np.array): Transformation matrix of Robosense Lidar to the body.
        cam_transform (np.array) : Transfroamtion matrix of camera to the body.
        P (np.array) : Camera projection matrix.
        """
    ...
    def angle_point(self, x, K, IMG_W):
        """
        Calculates the angle at which the point is situated from the given pixel x value.
