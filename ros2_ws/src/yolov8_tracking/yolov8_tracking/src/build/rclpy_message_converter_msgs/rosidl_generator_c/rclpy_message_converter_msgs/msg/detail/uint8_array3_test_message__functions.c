  ...
  rclpy_message_converter_msgs__msg__Uint8Array3TestMessage__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rclpy_message_converter_msgs__msg__Uint8Array3TestMessage * data =
      (rclpy_message_converter_msgs__msg__Uint8Array3TestMessage *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rclpy_message_converter_msgs__msg__Uint8Array3TestMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rclpy_message_converter_msgs__msg__Uint8Array3TestMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
