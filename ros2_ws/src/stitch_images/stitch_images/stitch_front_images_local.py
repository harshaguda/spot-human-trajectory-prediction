...
class ImagePreppedForOpenGL():
    ...
    def __init__(self, image_response):
        self.image_source_pinhole = True
        ...
        if self.image_source_pinhole:
            # Get the following from image info.
            resolution = numpy.asarray([image_response.source.cols, image_response.source.rows]) 

            focal_length = numpy.asarray([ image_response.source.pinhole.intrinsics.focal_length.x, image_response.source.pinhole.intrinsics.focal_length.y])
